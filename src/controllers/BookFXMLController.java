...
public class BookFXMLController implements Initializable{
  ...
  float pri;
            ...
            pri=Float.parseFloat(priceInput.getText());
            try 
        {  
            ConnectionClass connectionClass = new ConnectionClass();
            Connection connection = connectionClass.getConnection();
  ...
  myIndex = bookTable.getSelectionModel().getSelectedIndex();
        ...
        re = bookTable.getItems().get(myIndex).getRef();   
         tit = titleinput.getText();
            ...
            pri=priceInput.getText();
            System.out.println("price*********************** "+priceInput.getText());
        ...
        //    PreparedStatement  pst = con.prepareStatement("UPDATE livre SET title = ?,description = ?,author = ? WHERE ref = ?");
            PreparedStatement  pst = con.prepareStatement("update livre set title = ?,description = ? ,author = ?, Pages=?, price=?, year=?,  Language =? where ref = ? ");
		        ...
		        if (event.getClickCount() == 1 && (!myRow.isEmpty()))
		        {
		           ...
		           Descriptioninput.setText(bookTable.getItems().get(myIndex).getDescription());
                          ...
                          authorInput.setText(bookTable.getItems().get(myIndex).getAuthor());
                           ...
                           priceInput.setText(Float.toString(bookTable.getItems().get(myIndex).getPrice()));
